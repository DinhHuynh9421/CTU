...
--23
select count(*)
from(select tacgia,count(*) stop
from tacpham
group by tacgia having count(*)>1;)
--24
select tp.tua, count(*) from muon m join sach s on m.ns=s.ns
join tacpham tp on s.nt=tp.nt
group by tp.tua having count(*)=(select max(count(*)) from muon m1 join sach s1 on m1.ns=s1.ns
  join tacpham tp1 on s1.nt=tp1.nt group by tp1.tua);
--25 
select tp.tua, count(*)
from muon m join sach s on s.ns=m.ns
join tacpham tp on tp.nt=s.nt
group by tp.tua having count(*)=(select min(count(*)) from muon m1 join sach s1 on s1.ns=m1.ns
join tacpham tp1 on tp1.nt=s1.nt
group by tp1.tua);
--26
select m.nd, count( tp.nt) solanmuon
from muon m
join sach s on s.ns=m.ns
join tacpham tp on tp.nt=s.nt
group by m.nd having count(tp.nt)=(select max(count(tp.nt))
from muon m1 join sach s1 on s1.ns=m1.ns
join tacpham tp1 on tp1.nt=s1.nt
group by m1.nd);
--27
select m.nd, count( tp.nt) solanmuon
...
join sach s on s.ns=m.ns
join tacpham tp on tp.nt=s.nt
GROUP BY m.nd having count( tp.nt)=(select min(count( tp.nt))
from muon m1 join sach s1 on s1.ns=m1.ns
join tacpham tp1 on tp1.nt=s1.nt
group by m1.nd);
--28
select tua from tacpham t join sach s on s.nt=t.nt
left join muon m on s.ns=m.ns
where ngaymuon is null;
--29
select tua 
from tacpham
where nt not in(select nt from sach s left join muon m on m.ns=s.ns
where ngaymuon is null);
--30
select ho, ten
