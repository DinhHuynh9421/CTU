...
BEGIN 
 -- M? cursor 
 OPEN c_Emp; 
 -- Dùng vòng l?p ?? l?y h?t d? li?u 
 LOOP 
 -- L?y m?t dòng d? li?u t? cursor 
 FETCH c_Emp INTO v_Emp; 
 -- Thoát khoi dong lap neu da lay het du lieu trong 
cursor 
 ...
 -- C?p nh?t l?i ti?n th??ng trong b?ng Emp v?i mã nhân 
viên 
 -- trong cursor hi?n hành 
 UPDATE Emp 
 SET comm = v_Emp.comm 
...
CREATE TABLE EMP_RETIRE ( 
 ...
 DECLARE 
 -- Tìm các nhân viên có s? n?m làm vi?c >=28, khóa b?ng 
 -- emp khi cursor này ???c m?
 ...
 FOR UPDATE; 
BEGIN 
 -- bi?n emp_rec ???c dùng tr?c ti?p ?? l?y t?ng dòng d? li?u 
 -- trong cursor,ta dùng cách này thay cho vi?c khai báo bi?n 
và 
 -- dùng l?nh FETCH 
 FOR emp_rec IN cEmpRetire 
 LOOP 
 -- L?y dòng hi?n hành c?a cursor ?? thêm vào b?ng retire 
...
emp_rec.Deptno); 
 -- Xóa d? li?u trong Emp v?i m?u tin gi?ng v?i 
 -- m?u tin trong cursor hi?n hành 
 DELETE FROM Emp 
...
--c)
DECLARE 
 --Khai báo bi?n manager có ki?u d? li?u gi?ng v?i c?t MGR 
 --trong b?ng EMP 
 manager EMP.MGR%TYPE; 
 --Khi báo cursor có tham s? truy?n vào là mã s? c?a manager 
 --và khóa c?t SAL trong b?ng EMP l?i khi cursor này ???c m?
 ...
 FOR UPDATE of SAL; 
BEGIN 
 --Tìm mã s? c?a nhân viên tên là KING l?u vào bi?n manager 
 SELECT empno INTO manager 
 FROM EMP 
 WHERE EName = 'KING'; 
 --Khai báo không t??ng minh bi?n emp_rec tr?c ti?p trong 
 --vòng l?p for ?? l?u giá tr? c?a cursor hi?n hành 
 --ta dùng cách này thay cho vi?c khai báo t??ng minh m?t 
bi?n 
 --sau ?ó dùng l?nh FETCH ?? chuy?n d? li?u vào bi?n ?ó 
